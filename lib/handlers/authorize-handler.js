...
AuthorizeHandler.prototype.generateAuthorizationCode = function() {
  if (this.model.generateAuthorizationCode) {
    var model = this.model, context = this.context;
    return Promise.try(function () { return model.generateAuthorizationCode(context); });
  }
...
AuthorizeHandler.prototype.getClient = function(request) {
  ...
  var model = this.model, context = this.context;
  return Promise.try(function () { return model.getClient(clientId, context); })
...
AuthorizeHandler.prototype.saveAuthorizationCode = function(authorizationCode, expiresAt, scope, client, redirectUri, user) {
  ...
  var model = this.model, context = this.context;
  return Promise.try(function () { return model.saveAuthorizationCode(code, client, user, context); });
};
...
AuthorizeHandler.prototype.getResponseType = function(request, code) {
  ...
  if (!_.includes(['code'], responseType)) {
